...
`{score,level,limiting,criteria,reasons,forecastLeadHours}`, а также `best` (id показанных лучших окон:
...
критерий получает `score` 0–8 против заявленного свидетельства, сводный `score`
равен **минимуму** по критериям, `level` — `high` при 6–8, `medium` при 4–5, `low`
при 0–3, а `limiting` перечисляет критерии на этом минимуме. Критерий несёт
...
«последнее наблюдение сохраняется», получает по `forecast` не выше 4, даже когда
прогноз SWPC его подтверждает: 4 — короткая, 3 — средняя, 2 — длинная экстраполяция.
`forecastLeadHours` хранит непрерывное расстояние от последнего замера до начала окна.
Подтверждение градуировано: если горизонт выпуска
