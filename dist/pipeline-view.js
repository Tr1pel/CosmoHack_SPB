...
export function pipelineSummary(plan,data,selected=plan.recommended){
  ...
  return `<section class="panel summary-panel"><div class="summary-main"><div class="eyebrow">РЕКОМЕНДАЦИЯ</div><h2>${heading}</h2><div class="summary-time"><span class="rec-time">${range(w)}<span>UTC</span></span><span class="badge ${w.status}"><span class="dot"></span>${VERDICT[w.status]}</span></div><div class="rec-date">${esc(date(w.start))} · ${plan.request.duration} ч · место 1 из ${total}${w.id===a.id?' · исходное окно':` · сдвиг +${Math.round((w.start-a.start)/3600000)} ч`}</div><p>${esc(lead)}</p><div class="window-pills"><span>${plan.goodCount?`Все хорошие окна по порядку выбора (${plan.goodCount}):`:'Два лучших из доступных:'}</span>${best.map(pill).join('')}</div><p class="muted">${esc(original)}</p></div><aside class="summary-confidence level-${c.level}"><span>Уверенность оценки</span><strong>${confidenceText(c)}<em>${esc(score(c))}</em></strong><p class="conf-meaning">${esc(MEANING[c.level]??'')}. Это не вероятность безопасности.</p>${weakest.length&&c.score<SCORE_MAX?`<p>Ограничивают: ${weakest.map(x=>esc(x.label)).join(', ')}.</p>`:''}<button type="button" class="text-button" data-window="${esc(w.id)}" data-scroll="confidence">Критерии оценки ↓</button></aside></section>`;
}
...
export function pipelineHistory(plan,data){return `<section class="panel pipeline-summary"><h2>Исторические выпуски и независимая сверка</h2><p>Датированных выпусков на срезе: ${data.context.length}. Строгая воспроизводимость полного решения: ${plan.strictReproducibility?'да':'не подтверждена'}.</p>${data.context.map(r=>`<details><summary>${esc(r.sourceId)} · ${esc(r.publishedAt)}</summary><pre>${esc(typeof r.payload==='string'?r.payload:JSON.stringify(r.payload,null,2))}</pre></details>`).join('')}</section>`;}
